...
void strobe_FPGA (void)											//strobes the data onto the FPGA
{
	flash_pin_on();
	LCD_delay_us(1);
	flash_pin_off();
...
short data_mask (unsigned short ADC_input)	//masks the input data from the ADC to get it in the right form for the FPGA board
{
	short ADC_8_bit; 													//creates a short called ADC_8_bit
	ADC_input &= 0xFF0; 											//masks the 4 least significant bits of the 12 bit value
	...
	if ( control ==0) 												//if control = o
	{
		...
		for (int bit_num = 0; bit_num		< 4; bit_num ++)
		{
			...
			if (bit_num == 0)
			{
				if (bit_value == 1)
				{
					GPIOB -> BSRR |= (1u<<15); 				//if the lsb is 1 set the data line high
				}
				else if (bit_value == 0)
				{
					GPIOB -> BSRR |= (1u<<(15+16));		//if the lsb is 0 set the data line low  
				}
			}
			else if (bit_num == 1)
			{
				if (bit_value == 1)
				{
					GPIOB -> BSRR |= (1u<<12);				//if the lsb is 1 set the data line high
				}
				else if (bit_value == 0)
				{
					GPIOB -> BSRR |= (1u<<(12+16));		//if the lsb is 0 set the data line low
				}
			}
			else if (bit_num == 2)
			{
				if (bit_value == 1)
				{
					GPIOB -> BSRR |= (1u<<10);				//if the lsb is 1 set the data line high
				}
				else if (bit_value == 0)
				{
					GPIOB -> BSRR |= (1u<<(10+16));		//if the lsb is 0 set the data line low 
				}
			}
			else if (bit_num == 3)
			{
				if (bit_value ==1)
				{
					GPIOB -> BSRR |= (1u<<2);					//if the lsb is 1 set the sata line high
				}
				else if (bit_value == 0)
				{
					GPIOB -> BSRR |= (1u<<(2+16));		//if the lsb is 0 set the data line low
				}
			}
	...
	else if ( control == 1) 									//if control = 1
	{
		...
		for (int bit_num; bit_num		< 4; bit_num ++)
		{
			...
			else if (bit_num == 3)
			{
				if (bit_value == 1)
				{
					GPIOB -> BSRR |= (1u<<2);					//if the lsb is 1 set the data line high
				}
				else if (bit_value == 0)
				{
					GPIOB -> BSRR |= (1u<<(2+16));		//if the lsb is 0 set the data line low
